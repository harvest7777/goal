...
export function useDailyChartData(goals: Goal[]|null|undefined, date: Date| undefined) {
    ...
    useEffect(()=>{
        ...
        try {
            const fetchData = async () => {
                ...
                setGoalData({ goalToChartData, goalToTotalMins, goalToTargetMins, totalMinsWorkingThisDay });
    ...
    const goalToChartData = await Promise.all(
        goals.map(async (goal) => {
            ...
            const chartData = data.map((d: {  day_time: number }, index: number) => {
                ...
                return {
                    x: index,
                    value: shouldCutOff(index) ? null : runningSum,
                };
            });
