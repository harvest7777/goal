...
export default function DayProgressPage({
  ...
  return (
    <div
      className={`${className} w-full flex flex-col items-center justify-center gap-5`}
    >
      ...
      {goalData.totalMinsWorkingThisDay > 0 ? (
        <div className="w-full">
            ...
            <h2 className="text-left w-full mb-3">
              ...
              <span className="font-bold">
                {formatMinutesToHoursAndMinutes(
                  goalData.totalMinsWorkingThisDay
                )}
              </span>
            </h2>
            <BarGraph
          ...
          <div className={`mt-5 grid sm:grid-cols-2 md:grid-cols-3  gap-5`}>
            {goalData
              .goalsToRender!.filter((goal) => goal.daily_commitment !== null)
              .map((goal) => (
                <div key={goal.id}>
                  <Card
                    key={goal.id}
