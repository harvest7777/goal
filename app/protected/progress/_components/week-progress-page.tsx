...
export default function WeekProgressPage({
  display,
  className,
  date,
}: WeekProgressPageProps) {
  const { goalData, chartConfig, loading, xFormatter, yFormatter } =
    useWeeklyChartData(date);
  ...
  if (!date || loading || !goalData) {
    return (
      <div
        className={`h-[calc(50vh)] flex flex-col items-center align-middle justify-center gap-5`}
      >
        <Spinner />
      </div>
    );
          ...
          <h2 className="w-full">
            ...
            <span className="font-bold">
              {formatMinutesToHoursAndMinutes(
                goalData.totalMinsWorkingThisWeek
              )}
            </span>
            <span>
              {" "}
              ...
              {isThisWeek(date)
                ? "this week"
                : "the week of " + format(date, "MMMM do").toLowerCase()}
            </span>
          </h2>

          <div className={`mt-5 grid sm:grid-cols-2 md:grid-cols-3  gap-5`}>
            {goalData
              .goalsToRender!.filter((goal) => goal.weekly_commitment !== null)
              .map((goal) => (
                <div key={goal.id}>
                  <Card
                    key={goal.id}
                    className={`bg-background w-64 ${
                      !goal.is_focused && "opacity-30"
                    }`}
                  >
                    <CardHeader>
                      <CardTitle className="flex justify-between">
                        ...
                        <span className="text-muted-foreground font-normal">
                          {formatMinutesToHoursAndMinutes(
                            goalData.goalToTotalMins[goal.id]
                          )}
                        </span>
                      </CardTitle>
                    </CardHeader>
